...
public class Edge {
    ...
    public void drawEdge(Graphics g) {
        ...
        g.drawLine(start.center.x, start.center.y, end.center.x, end.center.y);
        //verify pressed button
            double angle = Math.atan2(this.end.center.y - this.start.center.y, this.end.center.x - this.start.center.x);
            int arrowLength = 10;  // You can adjust this value

            // Adjust position of arrowhead based on node radius
            int adjustedEndX = this.end.center.x - (int)(Frame.getNode_diam()/2 * Math.cos(angle));
            int adjustedEndY = this.end.center.y - (int)(Frame.getNode_diam()/2 * Math.sin(angle));
